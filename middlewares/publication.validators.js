...
export const isPublicationAuthor = async (req, res, next) => {
    try {
        ...
        if (!id) return res.status(400).send({ message: ' ID publication is required', success: false })
        const publication = await Publication.findById(id)
        if (!publication) return res.status(404).send({ message: 'Publication not found', success: false })
        if (publication.author.toString() !== userId.toString()) return res.status(403).send({ message: 'You are not the author', success: false })
